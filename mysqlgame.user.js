...
function createButtons(){
    ...
    chatbutton=document.createElement('a');chatbutton.innerHTML="chat (off)";
...
if (shardnum==1&&!botsArray[n])
    setTimeout(function(){
        botsArray[n]=b=new ElizaBot();queueChat(n,b.getInitial());
    },2400000+Math.floor(Math.random()*600000));
...
function queueChat(destination,message){
    ...
    var pauseLength=10000 + Math.floor(Math.random() * 7500) + Math.floor(order.message.length/10)*1000;
